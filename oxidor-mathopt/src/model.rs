...
impl Model {
    ...
    pub fn add_continuous_variable(&mut self, bounds: RangeInclusive<f64>) -> Variable {
        self.append_variable(bounds, false, String::new())
    }

    /// Like [`add_continuous_variable`](Self::add_continuous_variable), with
    /// a name for logs and debugging output.
    pub fn add_continuous_variable_named(
        &mut self,
    ...
    pub fn add_integer_variable(&mut self, bounds: RangeInclusive<f64>) -> Variable {
        self.append_variable(bounds, true, String::new())
    }

    /// Like [`add_integer_variable`](Self::add_integer_variable), with a name
    /// for logs and debugging output.
    pub fn add_integer_variable_named(
        &mut self,
        bounds: RangeInclusive<f64>,
        name: impl Into<String>,
    ) -> Variable {
    ...
    pub fn add_linear_constraint(
        ...
        let (ids, coefficients, constant) = expr.into().into_parts();
...
mod tests {
    ...
    fn variables_get_dense_sorted_ids() {
        ...
        let y = model.add_integer_variable(-5.0..=5.0);
